...
size_t binary_search(int low, int high, const std::vector<T>& elements, const C& val) {
    if (low > high) return low;
    int mid = (low + high) / 2;
    if (elements[mid] == val) return mid;
    else if (elements[mid] < val) return binary_search(mid + 1, high, elements, val);
    else return binary_search(low, mid - 1, elements, val);
}
...
{
    return binary_search(0, elements.size() - 1, elements, val);
}
