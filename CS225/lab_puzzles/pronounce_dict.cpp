...
{
    std::string word1_upper;
    std::string word2_upper;
    std::transform(word1.begin(), word1.end(), back_inserter(word1_upper), ::toupper);
    std::transform(word2.begin(), word2.end(), back_inserter(word2_upper), ::toupper);
    std::vector<std::string> word1_pro;
    std::vector<std::string> word2_pro;

    bool w1_found = false;
    bool w2_found = false;

    for (const std::pair<const std::string, std::vector<std::string>> & word : dict) {
        if (word.first == word1_upper) {
            word1_pro = word.second;
            w1_found = true;
            // std::cout << "[Word1] " << word.first << std::endl;
            // std::cout << "[Pron1] ";
            // for (size_t i = 0; i < word.second.size(); i++) {
            //     std::cout << word.second[i];
            // } std::cout << std::endl;
        } else if (word.first == word2_upper) {
            word2_pro = word.second;
            w2_found = true;
            // std::cout << "[Word2] " << word.first << std::endl;
            // std::cout << "[Pron2] ";
            // for (size_t i = 0; i < word.second.size(); i++) {
            //     std::cout << word.second[i];
            // } std::cout << std::endl;
    ...
    if (word1_pro == word2_pro && w1_found && w2_found) {
        return true;
    }
    return false;
}
