                       ...
                       vector<TileImage>& theTiles)
{
    ...
    for (int i = 0; i < (int)theTiles.size(); i ++) {
        ...
        Point<3> color = convertToLAB(pixel);
        tile_avg_map[color] = i;
        colors.push_back(color);
    }
    KDTree<3> tree(colors);
    for (int row = 0; row < theSource.getRows(); row ++) {
        for (int col = 0; col < theSource.getColumns(); col ++) {
            TileImage *temp = get_match_at_idx(tree, tile_avg_map, theTiles, theSource, row, col);
            result->setTile(row, col, temp);
        }
    }
    return result;
}
