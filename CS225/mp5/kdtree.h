...
class KDTree
{
  ...
  public:
    ...
    void conRecursive(vector<Point<Dim>> &points, int tl, int tr, int dimension);

    void sorting(vector<Point<Dim>> &points, int tl, int tr, int dimension, int median);

    void swap(vector<Point<Dim>> &points, int first, int second);

    void treeRecursive(vector<Point<Dim>> &points, int tl, int tr, KDTreeNode **subroot);

    void findHelper(const Point<Dim> &query, Point<Dim> &result, bool &checkFirst, KDTreeNode *subroot, int dimension) const;

    // void clear(KDTreeNode *subroot);
    //
    // void copy(KDTreeNode *newRoot, KDTreeNode *subroot);
    /**
