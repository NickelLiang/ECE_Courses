/*This is a program to optimize a circuit design.
ECE 220 MP 11, 11/27/17
...
int is_leaf_node(node_t* ptr) {
    if (ptr->module != NULL) { //If it has a module, then it is a leaf node
        return 1;
    } else {
        return 0;
    }
...
int is_internal_node(node_t* ptr) {
    if (ptr->cutline != UNDEFINED_CUTLINE) { //If it has a cutline, then it is a internal node
        return 1;
    } else {
        return 0;
    }
...
int is_in_subtree(node_t* a, node_t* b) {
    if (b == NULL) { //Base case 1: a is not a parent node of b
        return 0;
    } else if (b == a) { //Base case 2: a is a parent node of b
        return 1;
    } else { //Recursive: trace back b's parent
...
void rotate(node_t* ptr) {
    int temp = ptr->module->w; //Temporary value since we are operating pointers
...
void recut(node_t* ptr) {
    ...
    if (ptr->cutline == H) { //If it's cutline is H, change to V
        ptr->cutline = V;
    } else { //Vice versa
        ptr->cutline = H;
    }
...
void swap_module(node_t* a, node_t* b) {
    ...
    module_t *temp_module = a->module; //Temporary value since we are operating pointers
    cutline_t temp_cutline = a->cutline; //Temporary value since we are operating pointers
    a->module = b->module; //Swao their data
    a->cutline = b->cutline;
    b->module = temp_module;
    b->cutline = temp_cutline;
}
...
void swap_topology(node_t* a, node_t* b) {
    ...
    if (b->parent->left == b) { //Change their parent nodes' left/right pointer first
        b->parent->left = a;
    } else {
        b->parent->right = a;
    }
    ...
    node_t *temp_parent = a->parent; //Swap their own parent pointer
    a->parent = b->parent;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
    ...
    postfix_traversal(ptr->left, nth, expression); //Do left first
    postfix_traversal(ptr->right, nth, expression); //Then do right

    if (is_leaf_node(ptr)) { //Do the root
...
node_t* init_slicing_tree(node_t* par, int n) {
    ...
    // ptr->module = (module_t*)malloc(sizeof(module_t));
